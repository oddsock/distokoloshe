...
3. Restart all services with TLS enabled (including the certbot renewal container)
...
Certbot runs as a persistent container (under the `production` profile) and checks for renewal every 12 hours. Certificates are stored in a Docker volume (`distokoloshe_certs`) shared read-only with the web and livekit containers.
...
| `LIVEKIT_API_KEY` | Yes | `distokoloshe` | LiveKit API key |
