...
export function SignalStrength({ stats, serverCity }: SignalStrengthProps) {
  ...
  return (
    ...
    >
      {/* Bars */}
      <div className="flex items-end gap-[2px]" title={tier.label}>
        {BAR_HEIGHTS.map((h, i) => (
          <div
            key={i}
      ...
      {hover && (
        <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-xs text-zinc-300 whitespace-nowrap z-50 shadow-lg">
          <p className="font-semibold text-white mb-1">{tier.label}</p>
          {rttMs !== null ? (
            <>
