...
export function ScreenShareView({ publication, participantName, hasAudio, audioMuted, onToggleAudioMute, onDismiss }: ScreenShareViewProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center px-3 py-1.5 bg-zinc-200 dark:bg-zinc-800 text-xs text-zinc-500 dark:text-zinc-400 shrink-0 ${
        isFullscreen ? '' : 'rounded-t-xl'
      }`}>
        <div className="flex items-center gap-2">
          <Monitor size={14} />
          <span>{participantName === 'You' ? 'You are sharing your screen' : `${participantName} is sharing their screen`}</span>
        </div>
      </div>
      ...
      <div className={`relative ${isFullscreen ? 'flex-1 min-h-0' : ''}`}>
        <div className={`overflow-hidden ${isFullscreen ? 'h-full' : 'rounded-b-xl'}`}>
          <VideoTrackView
            ...
            fit="contain"
            className={isFullscreen ? '' : 'max-h-[70vh]'}
          />
        </div>
